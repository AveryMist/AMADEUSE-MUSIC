...
class PlayerController extends GetxController
  ...
  final lyricUi = UINetease(
    highlight: true,
    defaultSize: GetPlatform.isDesktop ? 22 : 20,
    defaultExtSize: GetPlatform.isDesktop ? 14 : 12,
  );
  ...
  /// Load lyrics automatically when a new song starts
  Future<void> _loadLyrics() async {
    ...
    try {
      ...
      if (lyricsR != null) {
        ...
        // Auto-enable synced lyrics if available
        if (lyricsR["synced"] != null && lyricsR["synced"].toString().isNotEmpty) {
          lyricsMode.value = 0; // Set to synced mode
        } else if (lyricsR["plainLyrics"] != null && lyricsR["plainLyrics"].toString() != "NA") {
          lyricsMode.value = 1; // Set to plain mode
        }
