...
void printERROR(dynamic text, {String tag = "amadeuse Music"}) {
  if (kReleaseMode) return;
  debugPrint("\x1B[31m[$tag]: $text\x1B[0m");
}

void printWarning(dynamic text, {String tag = 'amadeuse Music'}) {
  if (kReleaseMode) return;
  debugPrint("\x1B[33m[$tag]: $text\x1B[34m");
}

void printINFO(dynamic text, {String tag = 'amadeuse Music'}) {
  if (kReleaseMode) return;
...
void sortSongsNVideos(
  ...
  switch (sortType) {
    ...
    case SortType.Duration:
      ...
      break;
    case SortType.AdditionOrder:
      // For addition order, we use the index from extras if available
      // This assumes the index was stored when the song was added to playlist
      compareFunction = (a, b) {
        final aIndex = a.extras?['playlistIndex'] ?? 0;
        final bIndex = b.extras?['playlistIndex'] ?? 0;
...
Future<bool> newVersionCheck(String currentVersion) async {
  try {
    final tags = (await Dio()
            .get("https://api.github.com/repos/anandnet/amadeuse-Music/tags"))
        .data;
