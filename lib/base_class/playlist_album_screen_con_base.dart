...
abstract class PlaylistAlbumScreenControllerBase extends GetxController {
  ...
  void fetchSongsfromDatabase(String id) async {
    ...
    // Iterate through box entries to preserve index order
    for (int i = 0; i < box.length; i++) {
      final item = box.getAt(i);
      if (item != null) {
        final mediaItem = MediaItemBuilder.fromJson(item);
        // Add playlist index to extras for chronological sorting
        final updatedExtras = Map<String, dynamic>.from(mediaItem.extras ?? {});
        updatedExtras['playlistIndex'] = i;

        final updatedMediaItem = mediaItem.copyWith(extras: updatedExtras);
        songs.add(updatedMediaItem);
      }
    }
